...
//            return number_format($this->price_per_m2, '0', '',' ');
    ...
    public function getProductImage($th = 1)
    {
        if($this->image) {
            return $this->image->thumb($th);
        } elseif($this->catalog->single_image()->count()) {
            return $this->catalog->single_image->thumb($th);
        } elseif($this->catalog->image) {
            return $this->catalog->image_src;
        } else {
            return self::NO_IMAGE;
        }
    }
