...
function serviceImageAttache(elem, e){
	$.each(e.target.files, function(key, file)
	{
		if(file['size'] > max_file_size){
			...
			renderImage(file, function (imgSrc) {
				var item = '<img class="img-polaroid" src="' + imgSrc + '" width="200" data-image="' + imgSrc + '" onclick="return popupImage($(this).data(\'image\'))">';
				$('#service-image-block').html(item);
			});
		}
	});
	$(elem).val('');
...
function serviceDel(elem, e){
	e.preventDefault();
	if (!confirm('Удалить услугу?')) return false;
