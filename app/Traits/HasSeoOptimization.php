...
trait HasSeoOptimization{
    ...
    public static $defaultTextTemplate = '{name} купить по выгодной цене';

    public static $defaultAnnounceTemplate = '';
...
//        }
    ...
    public function generateAnnounce() {
        $template = $this->getAnnounceTemplate();
        $this->announce = $this->replaceTemplateVariable($template);
    }
    ...
    public function getAnnounceTemplate($catalog_id = null) {
        if (!$catalog_id) $catalog_id = $this->catalog_id;
        $catalog = Catalog::find($catalog_id);
        if (!$catalog) return null;
        if (!empty($catalog->product_announce_template)) return $catalog->product_announce_template;
        if ($catalog->parent_id) return $this->getAnnounceTemplate($catalog->parent_id);

        if ($admin_template = Settings::get('default_announce_template')) {
            return $admin_template;
        } else {
            return self::$defaultTitleTemplate;
        }
