<div class="mob-nav" data-burger-menu="data-burger-menu">
    <div class="mob-nav__body">
        <button class="mob-nav__close btn-reset" type="button" aria-label="Закрыть меню" data-burger-menu-close="data-burger-menu-close">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                <g fill="none">
                    <circle cx="12" cy="12" r="9" fill="currentColor" fill-opacity="0.25" />
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.2" d="m16 8l-8 8m0-8l8 8" />
                </g>
            </svg>
        </button>
        <div class="mob-nav__logo">
            <div class="logo">
                <img class="logo__img" src="static/images/common/logo.svg" width="76" height="72" alt="Теплобаланс" />
                <span class="logo__label">ООО «Теплобаланс»</span>
            </div>
        </div>
        <div class="mob-nav__menu">
            <nav class="mob-nav__nav">
                <ul class="mob-nav__nav-list">
                    <li class="mob-nav__nav-item is-dropdown">
                        <a class="mob-nav__nav-link is-dropdown" href="#" data-burger-dropdown="data-burger-dropdown">
                            <span>О Компании</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="7" fill="none">
                                <path fill="currentColor" fill-rule="evenodd" d="M5 2.578 1.332 6.3 0 5.362 4.334.964A.937.937 0 0 1 5 .7c.257 0 .501.096.666.264L10 5.362 8.668 6.3 5 2.578Z" clip-rule="evenodd" />
                            </svg>
                        </a>
                        <ul class="mob-nav__dropdown list-reset" data-burger-submenu="data-burger-submenu">
                            <li>
                                <a href="#">О Компании</a>
                            </li>
                            <li>
                                <a href="#">Портфолио</a>
                            </li>
                            <li>
                                <a href="#">Вакансии</a>
                            </li>
                            <li>
                                <a href="#">Контакты и реквизиты</a>
                            </li>
                        </ul>
                    </li>
                    <li class="mob-nav__nav-item is-dropdown">
                        <a class="mob-nav__nav-link is-dropdown" href="#" data-burger-dropdown="data-burger-dropdown">
                            <span>Услуги</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="7" fill="none">
                                <path fill="currentColor" fill-rule="evenodd" d="M5 2.578 1.332 6.3 0 5.362 4.334.964A.937.937 0 0 1 5 .7c.257 0 .501.096.666.264L10 5.362 8.668 6.3 5 2.578Z" clip-rule="evenodd" />
                            </svg>
                        </a>
                        <ul class="mob-nav__dropdown list-reset" data-burger-submenu="data-burger-submenu">
                            <li>
                                <a href="#">Обслуживание коммерческих узлов учёта (УКУТ)</a>
                            </li>
                            <li>
                                <a href="#">Система автоматического регулирования тепла (САРТ)</a>
                            </li>
                            <li>
                                <a href="#">Поверка и ремонт оборудования</a>
                            </li>
                            <li>
                                <a href="#">Проектирование, монтаж, и реконструкция УКУТ</a>
                            </li>
                        </ul>
                    </li>
                    <li class="mob-nav__nav-item">
                        <a class="mob-nav__nav-link" href="javascript:void(0)">
                            <span>Прайс</span>
                        </a>
                    </li>
                    <li class="mob-nav__nav-item">
                        <a class="mob-nav__nav-link" href="javascript:void(0)">
                            <span>Контакты</span>
                        </a>
                    </li>
                </ul>
            </nav>
        </div>
        <div class="mob-nav__foot">
            <a class="mob-nav__phone" href="tel:+73432883055" title="Позвонить нам">+7 343 288-30-55</a>
            <a class="mob-nav__email" href="mailto:info@teplo-balans.ru" title="Отправить email">
						<span class="mob-nav__email-icon site-icon">
							<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" fill="none">
								<path fill="currentColor" fill-rule="evenodd" d="M13.333 3.167H2.667c-.56 0-1.043.35-1.24.842.068.013.133.04.193.081L8 8.556l6.38-4.466a.499.499 0 0 1 .192-.081 1.338 1.338 0 0 0-1.239-.842Zm1.334 1.943-6.38 4.466a.5.5 0 0 1-.574 0L1.333 5.11v7.39c0 .733.6 1.333 1.334 1.333h10.666c.734 0 1.334-.6 1.334-1.333V5.11Z"
                                      clip-rule="evenodd" />
							</svg>
						</span>
                <span class="mob-nav__email-label">info@teplo-balans.ru</span>
            </a>
            <div class="mob-nav__socials">
                <!--ul.socials.list-reset-->
                <ul class="socials list-reset">
                    <li class="socials__item">
                        <a class="socials__link" href="javascript:void(0)" title="Написать в Whatsapp">
									<span class="socials__email-icon site-icon site-icon--black">
										<svg xmlns="http://www.w3.org/2000/svg" width="14" height="15" fill="none">
											<path fill="currentColor" d="M11.951 2.542A6.952 6.952 0 0 0 7.024.5C3.171.5.05 3.611.05 7.451c0 1.216.341 2.43.927 3.452L0 14.5l3.707-.972a7.132 7.132 0 0 0 3.317.826c3.854 0 6.976-3.11 6.976-6.951-.049-1.799-.732-3.549-2.049-4.861Zm-1.56 7.389c-.147.388-.83.777-1.171.826a2.732 2.732 0 0 1-1.074-.049c-.244-.097-.585-.194-.975-.389C5.415 9.59 4.293 7.84 4.195 7.694c-.097-.097-.732-.923-.732-1.798s.44-1.264.586-1.458c.146-.195.341-.195.488-.195h.341c.098 0 .244-.049.39.292.147.34.488 1.215.537 1.264a.309.309 0 0 1 0 .291 1.017 1.017 0 0 1-.195.292c-.098.097-.195.243-.244.292-.098.097-.195.194-.098.34.098.194.44.729.976 1.215.683.584 1.22.778 1.415.875.195.097.292.049.39-.048.097-.098.439-.487.536-.681.098-.194.244-.146.39-.097.147.048 1.025.486 1.171.583.195.097.293.146.342.195.049.145.049.486-.098.875Z"
                                            />
										</svg>
									</span>
                        </a>
                    </li>
                    <li class="socials__item">
                        <a class="socials__link" href="javascript:void(0)" title="Написать в Telegram">
									<span class="socials__email-icon site-icon site-icon--black">
										<svg xmlns="http://www.w3.org/2000/svg" width="16" height="15" fill="none">
											<path fill="currentColor" d="m15.647.996-2.475 12.921s-.346.896-1.298.466L6.164 9.85l-.027-.013c.772-.717 6.753-6.287 7.015-6.54.404-.39.153-.623-.317-.328L4 8.778.59 7.592s-.536-.197-.588-.627c-.052-.43.606-.663.606-.663L14.505.656s1.142-.52 1.142.34Z"
                                            />
										</svg>
									</span>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</div>
